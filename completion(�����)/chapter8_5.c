...
struct globalfifo_dev{
	...
	struct mutex mutex;
	struct completion fifo_completion_r;
	struct completion fifo_completion_w;
...
static ssize_t globalfifo_read (struct file *filp, char __user *buf, size_t size, loff_t *off)
{
	...
	while(globalfifo_devp->current_len == 0) {
		...
		wait_for_completion(&globalfifo_devp->fifo_completion_w);
	...
	if(copy_to_user(buf, globalfifo_devp->mem, size)){
		...
		complete(&globalfifo_devp->fifo_completion_r);
		ret = size;
	}
...
static ssize_t globalfifo_write (struct file *filp, const char __user *buf, size_t size, loff_t *off)
{
	...
	while(globalfifo_devp->current_len == GLOBALFIFO_SIZE) {
		...
		wait_for_completion(&globalfifo_devp->fifo_completion_r);
	...
	if(copy_from_user(globalfifo_devp->mem + globalfifo_devp->current_len, buf, size)){
		...
		complete(&globalfifo_devp->fifo_completion_w);
		ret = size;
	}
...
static int __init globalfifo_init(void)
{
	...
	*/
	...
	init_completion(&globalfifo_devp->fifo_completion_r);
	init_completion(&globalfifo_devp->fifo_completion_w);
	return 0;
