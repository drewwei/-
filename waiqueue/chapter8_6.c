...
static int condition_r,condition_w;
...
static ssize_t globalfifo_read (struct file *filp, char __user *buf, size_t size, loff_t *off)
{
	...
	wait_event_interruptible(globalfifo_devp->r_wait, condition_r);
	condition_r = 0;
	mutex_lock(&globalfifo_devp->mutex);			
	}
...
static ssize_t globalfifo_write (struct file *filp, const char __user *buf, size_t size, loff_t *off)
{
	...
	while(globalfifo_devp->current_len == GLOBALFIFO_SIZE) {
		...
		/*切换进程之前释放互斥锁*/
	mutex_unlock(&globalfifo_devp->mutex);
	wait_event_interruptible(globalfifo_devp->w_wait, condition_w); //此时当发送kill -9终止该线程时会失败！
	condition_w = 0;
...
static unsigned int globalfifo_poll(struct file *filp, poll_table *wait)
{
	...
	poll_wait(filp, &globalfifo_devp->r_wait, wait); //将设备结构体中的r_wait和w_wait添加到轮询表中意味着因调用select而阻塞的进程可以被r_wait和w_wait唤醒
